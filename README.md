...
| **Module 3** — severity grading | **Trained** (resnet50 + GeM pooling, 30 epochs) | Validation QWK **0.9156** (APTOS internal 85/15 split — a training-time metric, not held-out sensitivity/specificity). Real referable-DR (grade≥2) sensitivity/specificity via `evaluate_referable_dr.m`: **IDRiD held-out test, n=101 — sensitivity 0.778, specificity 0.842**; **Messidor-2 external, n=162 — sensitivity 0.762, specificity 0.922**. Ablation finding: `grade01Rule.m` never independently committed to a grade on either evaluation set (it always deferred to the CNN — Track A found other lesions or Track B had ambiguous MA candidates on every image evaluated), so "combined" and "CNN-only" are numerically identical here; the rule pathway exists and is exercised in the code (see `gradeImage.m`), it simply didn't activate standalone on this held-out data. Full per-image predictions in `results/evaluate_referable_dr_results.mat` |
