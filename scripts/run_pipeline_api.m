...
function apiResult = buildApiResult(result, cfg, outDir)
    ...
    apiResult.findings = buildLesionFindings(result.trackA, result.trackB);
...
% buildFindings/landmarkCentroid/describeLocation moved to
% module4_explainability/buildLesionFindings.m - shared with
% generateAnnotatedReport.m so both report surfaces show identical
% per-lesion location/count/reliability for the same image.
