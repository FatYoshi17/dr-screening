...
if isdlarray(dlX) && ~isempty(dims(dlX)) && ~any(dims(dlX) == 'U') && forwardPyTorchFormat ~= ""
    ...
    switch forwardPyTorchFormat
        ...
        case {'BTC'} %from CBT
            iValidatePropagatedLabelAsExpected(MATLABFormat, "CBT");
            permDLTToReversePython = [1 3 2]; %CBT -> CTB
            permReversePythonToDLT = [1 3 2]; %CTB -> CBT
        case {'BCTS'} %SCBT
        ...
        case {'BT'} %BT
            iValidatePropagatedLabelAsExpected(MATLABFormat, "BT");
            permDLTToReversePython = [2 1]; %BT -> TB
            permReversePythonToDLT = [2 1]; %TB -> BT
        case {'TU'} %TU
            iValidatePropagatedLabelAsExpected(MATLABFormat, "TU");
            permDLTToReversePython = [1 2]; %TU -> TU
            permReversePythonToDLT = [1 2]; %TU -> TU
        otherwise 
...
elseif any(dims(dlX) == 'U')
    % The passed-in 'rank' argument is unreliable here: it's threaded as
    % a separate auxiliary scalar dlarray through MATLAB's own nested
    % dlnetwork/predict() dispatch (a custom-layer-within-custom-layer
    % boundary), which was directly confirmed (via before/after
    % instrumentation) to corrupt such threaded scalars - see
    % SegformerSequenceReduction.m's comments for the full diagnosis.
    % dlX itself is real data flowing through the graph, so its own
    % ndims() is always reliable regardless of what 'rank' says -
    % permute() never changes an array's ndims, so this is the correct
    % source of truth in every case, not a narrower workaround.
    trueRank = max(2, ndims(dlX));
    dlXPerm = permute(stripdims(dlX), fliplr(1:trueRank));
    dlXPerm = dlarray(dlXPerm, repmat('U',1,trueRank));
